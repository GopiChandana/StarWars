...
function HomePage() {
  ...
  const handleSearchquery=()=>{
    let urlsplitter=results && activequeryIndex>-1 && results[activequeryIndex].url.split("/")
  ...
  return (
    <div onKeyUp={handleKeyUp} >
      ...
      <div className={results.length===0 || query===""?"search-input" : "search-input search-results"}>
      <div className="x_div">
      <input  placeholder="Search by name" ref={inputref} onChange={(e)=>handlequery(e.target.value)}/>
      ...
      {(results.length>0 && query!=="" ? <div ref={scrollref} className={results.length>=4 ? "results_box" :"results_box height_optimization"}>
      {results.map((item,i)=>(
       
          <div className={i!==activequeryIndex?"results_innerbox":"results_innerbox results_innerbox1"}>
      ...
      </div> : (query.length>0) ? <p>Enter a Valid Character Name</p> : null )}
    </div>
  );
}
